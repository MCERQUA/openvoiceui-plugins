...
class HermesGateway(GatewayBase):
    ...
    def __init__(self):
        ...
        # Prompts that arrived WHILE a run was in flight (no-redirect policy,
        # Mike 2026-07-12: "multiple prompts don't redirect the one in flight").
    ...
    def queue_followup(self, message: str, session_key: str) -> bool:
        ...
        redirects the in-flight run. Multiple queued prompts batch into one
        follow-up turn on the same browser event queue. Returns False when
        no run is active (caller should note it for the next turn instead)."""
        with self._active_runs_lock:
            info = self._active_runs.get(session_key)
            if not info:
                return False
            self._pending_followups.setdefault(session_key, []).append(message)
        logger.info(f"Hermes: queued follow-up for session {session_key} (in-flight run NOT redirected)")
        return True

    def note_for_next_turn(self, message: str, session_key: str) -> None:
        """Carry a message that missed its run (run ended before interject
        landed) into the session history so the NEXT turn sees it — fixes the
        action=missed silent-drop (messages vanished, 2026-07-12)."""
        hist = self._session_history.setdefault(session_key, [])
        hist.append({"role": "user", "content": message})
        logger.info(f"Hermes: noted missed interject for next turn on session {session_key}")
    ...
    def stream_to_queue(
        ...
        **kwargs,
        ...
        try:
            ...
            # policy): instead of emitting the terminal event, CHAIN one
            ...
            if _followup is not None:
                logger.info(
                    f"Hermes: chaining {_followup.count(chr(10)) + 1} queued follow-up "
                    f"prompt(s) after completed run on session {session_key}"
                )
                threading.Thread(
                    target=self.stream_to_queue,
                    args=(event_queue, _followup, session_key),
            ...
            with self._active_runs_lock:
                ...
                # Error/steer paths never drained the follow-up queue (the
                # browser stream closes on an error event, so chaining there
                # would stream into the void). Carry any leftovers into the
                # session history so the NEXT turn sees them — never drop.
                _leftover = self._pending_followups.pop(session_key, None)
            if _leftover:
                self._session_history.setdefault(session_key, []).append(
                    {"role": "user", "content": "\n".join(_leftover)}
                )
                logger.info(
                    f"Hermes: carried {len(_leftover)} un-chained follow-up prompt(s) "
